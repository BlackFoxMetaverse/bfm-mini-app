...
class UpdateTokenService {
    ...
    try {
      ...
      const updatedUser = await User.findOneAndUpdate(
        ...
        [
          ...
          { $unset: ["_qaDate", "_qaCount", "_isNewDay", "_canAttempt", "_token"] },
        ],
        { new: true }
      );
  ...
  async spinWheel(userId: string, tokenEarned: number, res: Response) {
    try {
      ...
      // Update token and spin time
      user.token = (user.token || 0) + clampedToken;
  ...
  async bookReading(tokenData: LoginToken, res: any, readingToken?: string, bookId?: string) {
    try {
      ...
      const updatedUser = await User.findOneAndUpdate(
        ...
        {
          $inc: { token: BOOK_READING_REWARD },
          $set: { lastBookReadingRewardAt: now, readingSessionNonce: newNonce },
        },
        { new: true }
      );
