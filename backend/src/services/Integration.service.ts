...
const CHANNEL_USERNAME = "@bfm_academy";
...
class IntegrationService {
  private readonly TELEGRAM_REWARD_POINTS = 1000;
  private readonly TWITTER_VERIFY_WAIT_MINUTES = 15;
  private readonly SOCIAL_VERIFY_WAIT_MINUTES = 15; // same wait as Twitter
  private readonly SOCIAL_BUNDLE_REWARD_POINTS = 2000; // bundle when all 3 completed

  private async maybeAwardCompulsoryBundle(userDoc: any) {
    if (userDoc.compulsoryBundleAwarded)
      return { awarded: false, alreadyClaimed: true };

    // Prefer normalized socialTasks array if present
    const twitterCompletedFromArray = Array.isArray(userDoc.socialTasks)
      ? !!userDoc.socialTasks.find(
          (t: any) => t.platform === "twitter" && t.rewarded
        )
      : false;
    const twitterCompleted =
      twitterCompletedFromArray || !!userDoc.twitterFollowRewarded;
    ...
    const updated = await User.findOneAndUpdate(
      { _id: userDoc._id, compulsoryBundleAwarded: { $ne: true } },
      {
        $inc: { token: 2000 },
        $set: {
          compulsoryBundleAwarded: true,
          compulsoryBundleAwardedAt: new Date(),
        },
      },
      { new: true }
    );
    return updated
      ? { awarded: true, totalTokens: updated.token }
      : { awarded: false, alreadyClaimed: true };
  }

  private async maybeAwardOptionalBundle(userDoc: any) {
    if (userDoc.optionalBundleAwarded)
      return { awarded: false, alreadyClaimed: true };

    // Prefer normalized socialTasks array if present
    let optionalCompletedCount = 0;
    if (Array.isArray(userDoc.socialTasks) && userDoc.socialTasks.length) {
      const optionalPlatforms = new Set([
        "instagram",
        "medium",
        "linkedin",
        "discord",
    ...
    const updated = await User.findOneAndUpdate(
      { _id: userDoc._id, optionalBundleAwarded: { $ne: true } },
      {
        $inc: { token: 2000 },
        $set: {
          optionalBundleAwarded: true,
          optionalBundleAwardedAt: new Date(),
        },
      },
      { new: true }
    );
    ...
    try {
      ...
      if (user.telegramRewardClaimed) {
        ...
        return ResponseHandler.success(
          res,
          "Telegram follow already recorded",
          {
            alreadyClaimed: true,
    ...
    try {
      ...
      const tasks = [
        ...
        },
        {
          ...
          description: `Follow @${
            process.env.TWITTER_TARGET_USERNAME || "our Twitter account"
          }`,
          completed: !!user.twitterTaskStartedAt,
          completedAt: user.twitterTaskStartedAt || null,
        ...
        },
        {
          ...
          description: `Follow ${
            process.env.INSTAGRAM_TARGET_HANDLE || "our Instagram"
          }`,
        ...
        },
        {
          ...
          description: `Follow ${
            process.env.LINKEDIN_TARGET_HANDLE || "our LinkedIn"
          }`,
      ...
      return ResponseHandler.success(res, "Available tasks retrieved", {
        tasks,
      });
    } catch (error: any) {
      return ResponseHandler.internalError(
  ...
  async getSocialConnections(req: any, res: Response) {
    try {
      ...
      const connections = {
        ...
        twitter: {
          connected: !!user.twitterTaskStartedAt,
          taskStartedAt: user.twitterTaskStartedAt,
      ...
      return ResponseHandler.success(res, "Social connections retrieved", {
        connections,
      });
    } catch (error: any) {
      return ResponseHandler.internalError(
        res,
        "Failed to get social connections",
        error.message
      );
    }
  }
  ...
  async getCompletedTasksSummary(req: any, res: Response) {
    try {
      ...
      return ResponseHandler.success(
        res,
        "Completed tasks summary retrieved",
        summary
      );
    } catch (error: any) {
      return ResponseHandler.internalError(
        res,
        "Failed to get completed tasks summary",
        error.message
      );
    }
  }
  ...
  async getRewardsHistory(req: any, res: Response) {
    try {
      ...
      return ResponseHandler.success(res, "Rewards history retrieved", {
        ...
        totalRewards: rewardsHistory.reduce(
  ...
  async disconnectAllSocialAccounts(req: any, res: Response) {
    try {
      ...
      // Disconnect Twitter
      if (user.twitterTaskStartedAt) {
        user.twitterTaskStartedAt = undefined;
        disconnectedAccounts.push("twitter");
      }

      // Disconnect optional tasks via socialTasks
      const before = Array.isArray(user.socialTasks)
        ? user.socialTasks.slice()
        : [];
      const remaining = before.filter(
        (t: any) =>
          !["instagram", "linkedin", "medium", "discord"].includes(t.platform)
      );
      const removed = before.filter((t: any) =>
        ["instagram", "linkedin", "medium", "discord"].includes(t.platform)
      );
      if (removed.length > 0) {
      ...
      return ResponseHandler.internalError(
        res,
        "Failed to disconnect social accounts",
        error.message
      );
    }
  }
      ...
      const stats = {
        ...
        platforms: {
          ...
          bundle: {
            ...
            rewardAmount:
              (user.compulsoryBundleAwarded ? 2000 : 0) +
              (user.optionalBundleAwarded ? 2000 : 0),
          },
        },
      } as any;
  ...
  async startTwitterTask(req: any, res: Response) {
    try {
      ...
      return ResponseHandler.success(res, "Twitter task started", {
        startedAt:
          updated?.twitterTaskStartedAt || user.twitterTaskStartedAt || now,
        actionUrl,
  ...
  /**
   * Verify Twitter task with a 5-minute wait. If eligible, award 2000 points once.
   */
  async verifyTwitterTask(req: any, res: Response) {
    try {
      ...
      // Enforce wait window
      const startedAt = new Date(user.twitterTaskStartedAt);
      const now = new Date();
      ...
      if (elapsedMs < requiredMs) {
        ...
        return ResponseHandler.success(
          res,
          "Verification in process, please wait",
          {
            canVerify: false,
      ...
      const updatedUser = await User.findOneAndUpdate(
        { _id: user._id, twitterFollowRewarded: { $ne: true } },
        { $set: { twitterFollowRewarded: true } },
        { new: true }
      );
      await User.updateOne(
        { _id: user._id, "socialTasks.platform": "twitter" },
        {
          $set: {
            "socialTasks.$.rewarded": true,
            "socialTasks.$.rewardedAt": new Date(),
          },
        }
      );
      await User.updateOne(
  ...
  async startInstagramTask(req: any, res: Response) {
    try {
      ...
      const existingInstagramTask = user.socialTasks?.find(
      ...
      if (existingInstagramTask && existingInstagramTask.startedAt) {
        ...
        await User.updateOne(
          ...
          {
            $set: {
              "socialTasks.$.startedAt": existingInstagramTask.startedAt,
            },
          }
        );
      } else {
      ...
      return ResponseHandler.internalError(
        res,
        "Failed to start Instagram task",
        error?.message || error
      );
    }
  }
  ...
  async verifyInstagramTask(req: any, res: Response) {
    try {
      ...
      if (!refreshedUser) {
        ...
        return ResponseHandler.success(
          res,
          "Instagram task already completed",
          {
      ...
      return ResponseHandler.internalError(
        res,
        "Failed to verify Instagram task",
        error?.message || error
      );
    }
  }
  ...
  async getInstagramStatus(req: any, res: Response) {
    try {
      ...
      const igTask = Array.isArray(user.socialTasks)
        ? user.socialTasks.find((t: any) => t.platform === "instagram")
        : undefined;
      const startedAt = igTask?.startedAt
        ? new Date(igTask.startedAt)
        : undefined;
      ...
      return ResponseHandler.internalError(
        res,
        "Failed to get Instagram status",
        error?.message || error
      );
    }
  }
  ...
  async startLinkedInTask(req: any, res: Response) {
    try {
      ...
      const existingLinkedInTask = user.socialTasks?.find(
  ...
  async verifyLinkedInTask(req: any, res: Response) {
    try {
      ...
      return ResponseHandler.internalError(
        res,
        "Failed to verify LinkedIn task",
        error?.message || error
      );
    }
  }
  ...
  async getLinkedInStatus(req: any, res: Response) {
    try {
      ...
      const startedAt = user.linkedinTaskStartedAt
        ? new Date(user.linkedinTaskStartedAt)
        : undefined;
      const rewarded = !!user.linkedinTaskStartedAt;
  ...
  async getTwitterStatus(req: any, res: Response) {
    try {
      ...
      const startedAt = user.twitterTaskStartedAt
        ? new Date(user.twitterTaskStartedAt)
        : undefined;
      const rewarded = !!user.twitterTaskStartedAt;
  ...
  async startMediumTask(req: any, res: Response) {
    try {
      ...
      const existingMediumTask = user.socialTasks?.find(
        (task) => task.platform === "medium"
      );
  ...
  async getMediumStatus(req: any, res: Response) {
    try {
      ...
      const mTask = Array.isArray(user.socialTasks)
        ? user.socialTasks.find((t: any) => t.platform === "medium")
        : undefined;
      const startedAt = mTask?.startedAt
        ? new Date(mTask.startedAt)
        : undefined;
  ...
  async startDiscordTask(req: any, res: Response) {
    try {
      ...
      const existingDiscordTask = user.socialTasks?.find(
        (task) => task.platform === "discord"
      );
  ...
  async getDiscordStatus(req: any, res: Response) {
    try {
      ...
      const dTask = Array.isArray(user.socialTasks)
        ? user.socialTasks.find((t: any) => t.platform === "discord")
        : undefined;
      const startedAt = dTask?.startedAt
        ? new Date(dTask.startedAt)
        : undefined;
