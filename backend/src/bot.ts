...
const BOT_TOKEN =
  process.env.BOT_TOKEN || "8297710276:AAHp4WlRjQESzR3C_oF86XUPGV2NW_C6_FE";
  ...
  try {
    if (mongoose.connection.readyState !== 1) {
      ...
      const collections = await mongoose.connection.db
        .listCollections()
        .toArray();
      const userCollection = collections.find((c) => c.name === "newusers");
      console.log(
    ...
    console.warn(
      "⚠️ Mongo connect failed, continuing without DB:",
      e?.message || e
    );
    mongoOk = false;
  }
...
async function applyReferralOnce(joiner: any, referrerTelegramId: number) {
  if (!joiner || !referrerTelegramId)
    return { applied: false, reason: "bad_inputs" };
  if (joiner.telegramId === referrerTelegramId)
    return { applied: false, reason: "self" };

  const referrer = await User.findOne({
    telegramId: Number(referrerTelegramId),
  });
  ...
  const fresh = await User.findById(joinerId)
    .select("referredBy referedBy")
    .lean();
  const normalizedJoinerRef =
    (fresh as any)?.referredBy?.toString?.() ||
    (fresh as any)?.referedBy?.toString?.();
  const same =
    normalizedJoinerRef && normalizedJoinerRef === referrerId.toString();
...
bot.start(async (ctx) => {
  try {
    ...
    const refStrForWeb = (
      payload ? (payload.startsWith("r") ? payload.slice(1) : payload) : ""
    ).trim();
    const playNowUrl = `https://7tvmjdm0-5173.inc1.devtunnels.ms${
    ...
    await ctx.reply(introText, {
      ...
      ...Markup.inlineKeyboard([
        ...
        [
          ...
          Markup.button.url(
            "🟣 Discord",
            "https://discord.com/invite/jGtrk7TejJ"
          ),
        ],
        [
    ...
    const referrerIdStr = (
      payload.startsWith("r") ? payload.slice(1) : payload
    ).trim();
...
async function checkSpinAvailability(
  ...
  if (!lastSpin || now.getTime() - lastSpin.getTime() >= 24 * 60 * 60 * 1000) {
    ...
    const hoursLeft =
      24 - Math.floor((now.getTime() - lastSpin.getTime()) / (60 * 60 * 1000));
      ...
      try {
        ...
        if (spinCheck?.isAvailable) {
          ...
          await User.updateOne(
            { _id: user._id },
            { $set: { spinNotifiedAt: new Date() } }
          );
        }
      } catch (error: any) {
        if (!error?.message?.includes("blocked")) {
          console.error(
            `❌ Notification failed for user ${user.telegramId}:`,
            error?.message || error
          );
        }
      }
